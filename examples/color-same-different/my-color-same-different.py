...
def make_same_different_node(
    ...
    click_sensor = nk.sensors.ClickSensor(
        x=fixation_card.x,
        y=fixation_card.y,
        w=fixation_card.w, h=fixation_card.h,
    )

    fixation_node = nk.Node(
        cards={'fixation':fixation_card},
        sensors={
            #'fixated':click_sensor
            'fixated': nk.sensors.KeySensor(key=' ')
        },
    )
    ...
    report_same_card = nk.cards.TextCard(
    ...
    report_different_card = nk.cards.TextCard(
        background_color = to_hex(200, 200, 200),
        text= '**Different**',
        x=0,
        y=-0.45,
    ...
    stimulus_node = nk.Node(
        cards={
            ...
            #'same':report_same_card,
            #'different':report_different_card,
        },
        sensors = {
            #'same': nk.sensors.ClickSensor(
            #    x=report_same_card.x,
            #    y=report_same_card.y,
            #    w=report_same_card.w, h=report_same_card.h,
            #),
            #'different': nk.sensors.ClickSensor(
            #    x=report_different_card.x,
            #    y=report_different_card.y,
            #    w=report_different_card.w, h=report_different_card.h,
            #)
...
if __name__ == '__main__':
    ...
    for _ in range(50):
        ...
        if random.random() < 0.5:
            ...
            get_delta = lambda: random.random() * 50 - 25
            color2 = tuple(
                max(0, min(255, int(c + get_delta())))
                for c in color1
            )
