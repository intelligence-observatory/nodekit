...
def make_numerosity_trial(
    ...
    stimulus_card_card = nk.cards.ImageCard(
        x=0, y=0, w=0.5, h=0.5, image=image,
        start_msec=0,
    ...
    main_node = nk.Node(
        ...
        effects=[
            nk.effects.HidePointerEffect(
                start_msec=0, end_msec=free_text_card.start_msec,
            )
        ]
    )
...
if __name__ == '__main__':
    ...
    for image_path in images:
        trials.append(
            make_numerosity_trial(nk.assets.Image.from_path(image_path))
        )
