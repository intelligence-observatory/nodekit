...
- Distinguish core records/contracts from API response models. SQLModel records belong in `nodekit-server/nodekit_server/`; storage code and server concerns should stay server-local and must not leak into `nodekit/`.
