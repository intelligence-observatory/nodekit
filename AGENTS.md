...
- `nodekit-server/` is the optional backend service for turning Graphs into hosted participant-facing Sites, storing submissions/runs, and managing Assets. It may depend on `nodekit`, but DB, storage, FastAPI, and deployment concerns must not leak into the core NodeKit ontology.
