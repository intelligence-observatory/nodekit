...
- `src/nodekit/server/` contains shared public server/client API contracts, values, pagination helpers, and the Python client. These are importable by the Python client and by `nodekit-server`.
...
- Generated static directories such as `src/nodekit/_static` and `src/nodekit/server/dashboard/_static` contain builds of `nodekit-browser` and `nodekit-dashboard`; these should not be inspected by the Agent.
...
- The server should own Asset ingestion during Site creation. The Python client posts Graph JSON, preflights Assets, uploads missing Asset bytes, and the server verifies/stores those bytes, rewrites Asset locators to server-owned URLs, and stores the frozen Graph snapshot.
...
- Prefer the FastAPI + SQLModel stack for the server. Use `SQLModel.metadata.create_all(...)` for the canonical current schema rather than Alembic migration history in this repo.
...
- `make build-dashboard`: build `src/nodekit-dashboard/` and copy bundle to the packaged dashboard static directory.
- `make build`: run lint/check/test, rebuild browser and dashboard assets, build docs, and run `uv build` for the package.
...
- Schema guards: spatial fields use strict pixel integer aliases, hex colors normalize to `#rrggbbaa`, Assets require valid MIME and SHA-256, multi-select `max_selections >= min_selections`, waits > 0, sliders `num_bins > 1`. Expression evaluator enforces operand types, container types, bounds, and division-by-zero checks.
...
- Keep `uv.lock` and npm lockfiles aligned with dependency updates. Rerun `make build-browser` after `src/nodekit-browser/` changes and `make build-dashboard` after `src/nodekit-dashboard/` changes.
...
- NodeKit-owned UI should use Inter as its default sans-serif typeface, matching `nodekit-browser`.
