import pytest
from topo_sort import Node, Transition, topological_sort
...
def test_example_pass():
    ...
    result = topological_sort(nodes, transitions)
    
    # Check if all node indices are present:
    assert set(result) == set(range(len(nodes)))
    # Check that there are no duplicates
    assert len(result) == len(nodes)


def test_example_2_fail():
    ...
    with pytest.raises(ValueError, match="Loop present in Graph"):
        topological_sort(nodes, transitions)
