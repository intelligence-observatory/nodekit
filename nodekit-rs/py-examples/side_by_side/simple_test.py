...
cards = {
    "image": nk.cards.ImageCard(
        ...
        image=nk.assets.Image.from_path(ASSETS_DIRECTORY.joinpath("test_image.png").resolve()),
    ),
    "video": nk.cards.VideoCard(
        region=nk.Region(x=0, y=0, w=256, h=256, z_index=1),
        video=nk.assets.Video.from_path(ASSETS_DIRECTORY.joinpath("test-video.mp4").resolve()),
    ),
...
# TODO
node = nk.Node(
    card=card, sensor=nk.sensors.WaitSensor(duration_msec=10000), board_color=board_color
)
