...
impl Timers {
    pub fn add_card(&mut self, timer: Timer, card_key: CardKey) {
        ...
        self.entities.insert(timer_key, TimedEntityKey::Card(card_key));
    }

    pub fn add_sensor(&mut self, timer: Timer, sensor_key: SensorKey) {
        let timer_key = self.timers.insert(timer);
        self.entities.insert(timer_key, TimedEntityKey::Sensor(sensor_key));
    }
    
    pub fn tick(&mut self) -> TickResult {
        // Advance all timers.
        self.timers.values_mut().filter(|timer| timer.state != TimerState::Finished).for_each(|timer| timer.advance());
