...
# %%
def test_asset_resolver_redirects_for_public_url_storage(server_main: ModuleType) -> None:
    records: Any = sys.modules["nodekit_server.records"]
    deps: Any = sys.modules["nodekit_server.deps"]
    storage: Any = sys.modules["nodekit_server.storage"]
    sha256 = "f" * 64

    class PublicUrlAssetStore:
        def resolve(self, storage_key: str) -> Any:
            return storage.AssetResolution(
                redirect_url=f"https://cdn.example.com/{storage_key}",
            )

    with sqlmodel.Session(deps.engine) as session:
        session.merge(
            records.AssetRecord(
                sha256=sha256,
                media_type="image/png",
                size_bytes=1,
                storage_key=f"assets/{sha256}",
            )
        )
        session.commit()

    server_main.app.dependency_overrides[deps.get_asset_store] = PublicUrlAssetStore
    try:
        with TestClient(server_main.app) as client:
            response = client.get(f"/assets/{sha256}", follow_redirects=False)
    finally:
        server_main.app.dependency_overrides.pop(deps.get_asset_store, None)

    assert response.status_code == 307
    assert response.headers["location"] == f"https://cdn.example.com/assets/{sha256}"
