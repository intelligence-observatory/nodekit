...
def get_negative_node():
    return nk.Node(
        cards={},
        sensors={"wait": nk.sensors.WaitSensor(duration_msec=5000)},
    )
...
def test_multiple_roots_tie_break():
    nodes = {
        "A": nk.Node(cards={}, sensors={"a": nk.sensors.WaitSensor(duration_msec=1)}),
        "B": nk.Node(cards={}, sensors={"b": nk.sensors.WaitSensor(duration_msec=1)}),
        "C": nk.Node(cards={}, sensors={"c": nk.sensors.WaitSensor(duration_msec=1)}),
    }
    transitions = {
        "A": {"a": "C"},
        "B": {"b": "C"},
...
def test_invalid_sensor_reference():
    nodes = {
        "A": nk.Node(cards={}, sensors={"s": nk.sensors.WaitSensor(duration_msec=1)}),
        "B": nk.Node(cards={}, sensors={"s": nk.sensors.WaitSensor(duration_msec=1)}),
        "C": nk.Node(cards={}, sensors={"s": nk.sensors.WaitSensor(duration_msec=1)}),
    }
